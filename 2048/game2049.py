...
class play2048():
	...
	def strategy(self):
		...
		for dir in moves:
			method = getattr(self, '%sRule' % dir)
			resultant_list = method(actualList = self.numList, doScore=False)
			first_lists.append((dir, resultant_list))

		for x in first_lists[:]:
			print(x[1])
			if x[1] == self.numList:
				first_lists.remove(x)

		available_movement = [x for x,_ in first_lists]
		print(available_movement)

		for first_outcome in first_lists:
			for movement in available_movement:
				method = getattr(self, '%sRule' % movement)
				outcome = method(actualList = first_outcome[1],doScore=False)
				...
				if outcome != first_outcome:
					...
					for r in range(4):
						for c in range(4):
							if outcome[r][c] == 0:
								counter += 1
					second_lists.append((movement, counter))
		print('second list: ', second_lists)
		second_result = sorted(second_lists, key = lambda x: x[1], reverse= True)
		print('second result: ', second_result[0][0])

		return second_result[0][0]
