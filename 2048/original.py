...
class play2048():
	...
	def strategy(self):
		...
		for dir in moves:
			resultant_list = methods[dir](self.numList, doScore=False)
			if resultant_list != self.numList:
				counter = 0
				for r in range(4):
					for c in range(4):
						if resultant_list[r][c] == 0:
							counter += 1
				empty_tiles.append((dir,counter))

		result = [x for x,y in sorted(empty_tiles, key = lambda x: x[1], reverse= True)]
